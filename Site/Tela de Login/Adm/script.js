...
document.querySelector('button[type="submit"]').addEventListener('click', async function(event) {
    ...
    try {
      ...
      const url = "https://fjiwbrhjnmpuyhskhrlt.supabase.co/rest/v1/adm?cpf=eq." + cpf + "&select=*&senha=eq."+senha;
      ...
      if (data.length > 0) {
        ...
        window.location.href = '../../Tela Principal/Adm/index.html'; // Altere para a página desejada
      } else {
